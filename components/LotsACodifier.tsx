...
import {
  ...
  BoxIcon, KeyIcon, QuestionMarkCircleIcon,
} from './Icons';
...
const codesHeaders = [
  'NUMERO LOT', 'DATE SONDAGE', 'DATE CODIFICATION', 'CODIFIÉ PAR', 'SONDEUR', 'RAPPORT'
];

const LotsACodifier: React.FC<LotsACodifierProps> = ({ onNavigateBack }) => {
  return (
    <div className="p-6 lg:p-10 space-y-8 bg-gray-50">
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg border space-y-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-800 mb-2 flex items-center"><BoxIcon className="h-6 w-6 mr-2 text-blue-800" />Lots à Codifier</h3>
          ...
          <div className="overflow-x-auto border rounded-lg shadow-md">
            <table className="min-w-full text-xs">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                <tr><td colSpan={lotsHeaders.length + 2} className="text-center py-10"><p className="text-gray-500 font-semibold">Aucune donnée</p></td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        {/* Codes Secrets Table */}
        <div>
          <h3 className="text-lg font-bold text-gray-800 mb-2 flex items-center"><KeyIcon className="h-6 w-6 mr-2 text-blue-800" />Codes Secrets</h3>
           <p className="text-center text-sm text-gray-500 mb-2 italic">** Faites défiler horizontalement pour voir toutes les colonnes</p>
          <div className="overflow-x-auto border rounded-lg shadow-md">
            <table className="min-w-full text-xs">
              <thead className="bg-[#0d2d53] text-white uppercase">
                <tr>
                  ...
                  {codesHeaders.map(header => (
                     <th key={header} className="p-2 font-semibold tracking-wider text-left whitespace-nowrap">{header}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                <tr><td colSpan={codesHeaders.length + 1} className="text-center py-10"><p className="text-gray-500 font-semibold">Aucune donnée</p></td></tr>
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="mt-6 pt-6 border-t bg-blue-50 p-6 rounded-lg">
          <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center"><QuestionMarkCircleIcon className="h-6 w-6 mr-2 text-blue-700"/>Décision de Codification</h3>
          <div className="flex items-end gap-6">
            <div className="space-y-1 w-64">
              <label className="font-medium text-gray-600">Codifier ?</label>
        ...
        <div className="flex justify-end gap-4 mt-6 pt-6 border-t">
            ...
            <button className="bg-[#0d2d53] hover:bg-blue-800 text-white font-bold py-2 px-6 rounded-lg shadow-md transition-colors">Codifier la Sélection</button>
            <button className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg shadow-md transition-colors">Générer Codes</button>
            <button className="bg-white hover:bg-gray-100 text-gray-700 font-bold py-2 px-6 rounded-lg border border-gray-300 shadow-sm transition-colors">Exporter Liste</button>
            <button className="bg-white hover:bg-gray-100 text-gray-700 font-bold py-2 px-6 rounded-lg border border-gray-300 shadow-sm transition-colors">Init. Code du Jour</button>
        </div>
      </div>
